...
`--db` 指定 SQLite 文件，`--keyring` 指定 UTF-8 JSON 文件，形状为 `{"active_version":1,"keys":{"1":"<base64>","2":"<base64>","3":"<base64>"}}`。版本是正整数，密钥是各自独立的 32 字节随机值，编码采用标准 Base64。keyring 在启动时加载，也可通过受信任管理接口在线重载（见下）；文件属于外部配置，禁止把其中的密钥写入数据库或日志。已有配置校验行为需保留。
...
`POST /v1/keys/reload` 是受信任的本地管理接口，无请求正文、不需租户头。每次调用重新读取启动时 `--keyring` 指定的 UTF-8 JSON 文件，沿用启动时相同的版本、Base64 和 32 字节密钥校验。除此之外，新快照还必须：包含数据库当前持久活动版本；包含全部现有记录所引用的每个密钥版本；并且候选密钥必须能认证数据库中的全部现有信封（封装与正文）。文件不可读、JSON/结构非法、活动版本或任一引用版本缺失、或任一信封无法用候选密钥认证，统一返回 `400 {"error":"invalid_keyring"}`（不暴露具体原因或库错误）；存储读取失败仍为 `503 storage_error`。任何失败都不改变当前内存快照、持久活动版本、记录或审计链，修复文件后可原样重试。

成功时在一个串行时点一次性替换内存密钥快照，返回 `200 {"active_version":当前持久活动版本,"versions":[已加载版本升序]}`。文件中的 `active_version` 只校验格式（正整数）及其键存在，绝不覆盖数据库状态，因此它可以保持为旧的初始值。重载不写数据库、不产生审计事件。加入更高版本后可直接用于轮换；某版本已非活动版本且不再有任何记录引用时（例如完成轮换之后），可以从文件中删除对应密钥再重载，缩小在用密钥集合；仍被引用的版本不得删除。响应与日志只包含版本号，不含任何密钥材料。
...
审计事件存于 SQLite `audit_events` 表（`tenant,sequence` 联合主键，附 `kind` 及各事件字段），跨重启持久。审计读取与创建、轮换、清单、keyring 重载共享同一进程级串行锁，链上不会出现重号、断号或错链（重载本身不追加事件）。审计表写入失败与业务写入一样表现为 `503 storage_error`，业务记录与审计事件在同一事务内共同回滚；故障解除后服务恢复正常，链仍可完整校验。
...
同一进程中，并发创建同租户同 id 只能一个成功，其余返回 `409 conflict`。创建、读取、清单、轮换、审计读取和 keyring 重载的成功结果必须与某个完整串行顺序一致，不能观察半轮换或半重载状态。重载与并发的创建或轮换竞态时不得产生半更新、内部错误或暂时不可读：每个请求要么基于旧快照、要么基于新快照完整执行，结束后清单一致性、轮换原子性、租户隔离与审计链协议仍然成立。写入与成功轮换并发结束后，所有记录均应为最终活动版本且可读；两个相同目标版本的轮换并发执行时，一个完成实际轮换，另一个返回幂等空操作。数据量范围为本地小型账本，无需分页、跨进程协调、批次后台迁移或性能基准。
